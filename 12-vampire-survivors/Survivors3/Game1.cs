    ...
    protected override void UpdateGame(GameTime gameTime)
    {
        ...
        if (GameController.Stress)
        {
            for (int i = 0; i < 1000; i++)
                _swarm.Spawn(_random.Next(2), _spawner.PointAround(_player.Position));
        }
        ...
        using (_profiler.Measure("Spawn"))
        {
            ...
            for (int i = 0; i < count; i++)
                _swarm.Spawn(_spawner.PickKind(), _spawner.PointAround(_player.Position));
        }
